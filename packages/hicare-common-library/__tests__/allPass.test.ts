import { allPass } from 'lib/utility/allPass';
import { describe, expect, it } from 'vitest';
import { isNullish } from 'lib/utility/isNullish';
import { isString } from 'lib/utility/isString';
import { isArray } from 'lib/utility/isArray';
...
describe('allPass', () => {
    ...
    it('모든 함수에 대해 data가 조건을 만족하면 true 반환', () => {
        ...
        expect(allPass(null, [isNullish, isDivisibleBy3])).toBe(true);
    ...
    it('함수 중 하나라도 조건을 만족시키지 못하면 false 반환', () => {
        ...
        expect(allPass('', [isNullish, isDivisibleBy3])).toBe(false);
    });
});
